...
class Youku:
    ...
    def verfity_title(self, title, srcname):
        if title == srcname:
            return True
        else:
            return False

    def aliasnamecheck(self, temp):
        '''
        使用表格提供的又名爬取
        :param temp:
        :return:
    ...
    def dealname(self, chinesename):
        ...
        try:
            ...
            if not temp:
                ...
                result = self.aliasnamecheck(temp[0])
                return rs[0], result
    ...
    def main(self, name, year):
        ...
        if self.countnum >= 38:
            ...
            print("pre_verify:", pre_verity)
            if pre_verity:
                time.sleep(180)
                self.countnum = 38
                print("retry params:", name, year)
                temp = self.main(name, year)
        ...
        for item in items:
            ...
            if titleobj:
                title = titleobj.text
                verity_title = self.verfity_title(title, name)
                if verity_title:
    ...
    def start(self, name, year):
        namelst = self.dealname(name)
        first_search = self.main(namelst[0], year)
        if not first_search and namelst[1]:
            final_search = self.main(namelst[1], year)
            return final_search
        return first_search
...
if __name__ == '__main__':
    ...
    # e_name = Youku().start("医神（又名：三宝医馆/侠胆医神）（合拍剧集）", 2003)
    # print(">>>>>>>>>>>>:", e_name)
